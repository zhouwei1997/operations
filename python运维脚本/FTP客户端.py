...
class FTPClient:
    def __init__(self, host, user, passwd, port):
        ...
        self.passwd = passwd
        self.port = port
        self.res = {'ststus': True, 'msg': None}

    def _login(self):
        '''
        登录FTP服务器
        :return: 连接或登录出现异常时返回的错误信息
        '''
        try:
            self._ftp = FTP()
    ...
    def upload(self, localpath, remotepath=None):
        ...
        # 上传文件
        self.ftp.storbinary('STOR ' + remotepath, localpath)
        fp.close()
    ...
    def download(self, remotepath, localpath=None):
        ...
        # 如果为传递本地文件路径，则下载到当前目录，文件名称同远程文件
        if not localpath:
            localpath = path.basename(remotepath)
        # 如果localpath是目录的话和remotepath的basename拼接
    ...
    def rename(self, old_name=None, new_name=None):
        '''
        重命名
        :param old_name: 旧的文件或目录名称
        :param new_name: 新的文件或目录名称
        :return: 执行结果
        '''
