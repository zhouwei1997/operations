...
# linux日常巡检脚本
# 巡检包括磁盘、内存、cpu进程检查、文件修改、用户登录等信息

# 主机信息日常巡检
# eth33 网卡的名称
IPADDR=$(ifconfig ech33|grep 'inet addr'|awk -F '[ :]' '[print $13]')
# 设置系统的环境变量
export PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:/root/bin
source /etc/profile

# 查看执行脚本的用户是否为root用户，如果是则执行。否则输出提示信息
...
function version() {
    ...
    echo "逻辑CPU个数:$Virt_CPU"
    echo "每CPU核心数:$CPU_kernels"
    echo " CPU型号:$CPU_Type"
    echo " CPU架构:$CPU_Arch"
    #报表信息
    report_CPUs=$Virt_CPU #CPU数量
...
function getMemStatus() {
    ...
    let MemUsed=MenTotal-MemFree
    MemPercent=$(awk "BEGIN {if(MemTotal==0){printf 100}else{printf \"%.2f\",$MemUsed*100/$MenTotal}}")
    report_MemTotal="$((MemTotal/1024/1024))""GB" # 内存总容量（GB）
    report_MemFree="$((MemFree/1024/1024))""GB" # 内存总容量（GB）
    report_MemUsedPercent="$(awk "BEGIN {if($MemTotal==0){print 100}else{printf \"%.2f\",$MemUsed*100/$MenTotal}}")
    ""%"  #内存使用率%
}
...
function getDiskStatus() {
    ...
    df -hiP | sed 's/Mounted on/Mounted/' > /tmp/disk
    join /tmp/inode | awk '{print $1,$2,"|",$3,$4,$5,$6,"|",$8,$9,$10,$11,"|",$12}' | column -t
    # 报表信息
    diskdata=$(df -TP | sed 'id' | awk '$2!="tmpfs"{print}') #KB
...
function getListenStatus(){
    ...
    echo "$TCPListen"
    #报表信息
    report_Listen="$(echo "$TCPListen" | sed '1d' | awk '/tcp/ {print $5}' | awk -F: '{print $NF}' | sort | uniq | wc
     -l)"
}
...
function getUserLastLogin(){
    # 获取用户最近一次登录的时间，含年份
    # 由于last命令不支持显示年份，last -t YYYYMMDDHHMMSS 表示某个时间之间的登录
    # 对比今天之前和今年元旦之间某个用户登录的次数有变化，则说明最近一次登录是今年
    username=$1
    : ${username:="`whoami`"}
    thisYear=$(date +%Y)
...
echo -e "`date "+%Y-%m-%d %H:%M:%S"` 巡检报告"  | mail -a $RESULTFILE -s "hengyu19971023" h@163.com
